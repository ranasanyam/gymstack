...
//       member_id: string
//       amount: number
//       payment_method: 'cash' | 'gpay' | 'phonepe'
...
type MemberPaymentRow =
  Database['public']['Tables']['member_payments']['Row']

type MemberPaymentInsert =
  Database['public']['Tables']['member_payments']['Insert']

type MemberPaymentUpdate =
  Database['public']['Tables']['member_payments']['Update']

/* ======================================================
   FETCH GYM PAYMENTS
   (via gym_members relation)
====================================================== */
...
export function useMemberPaymentHistory(memberId?: string) {
  return useQuery({
    queryKey: ['member-payment-history', memberId],
    enabled: !!memberId,
    queryFn: async () => {
...
/* ======================================================
   MEMBER PAYMENTS (FOR MEMBER DASHBOARD)
====================================================== */
...
export function useGymRevenue(gymId?: string) {
  return useQuery({
    queryKey: ['gym-revenue', gymId],
    enabled: !!gymId,
    queryFn: async () => {
      ...
      const thisMonth =
        data?.filter(p => new Date(p.payment_date!) >= thisMonthStart)
          .reduce((sum, p) => sum + Number(p.amount), 0) ?? 0
...
export function useCreatePayment() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // ✅ Billing month = first day of current month
      const now = new Date()
...
export function useUpdatePaymentStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const updateData: MemberPaymentUpdate = { status }
